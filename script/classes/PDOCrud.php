<?php
/**
 do not tamper with the classes pliz ...usichange kitu apa
  ---------------------------------------------------------------------------------------------------------------
 */
Class PDOCrud {
    ...
    public function __construct($multi = false, $template = "", $skin = "") {
        ...
        $this->multi = $multi;
        if(!$this->pdocrudhelper->verifyPurchase($this->settings)){
            die();
        }
        if (!empty($template))
    ...
    private function initializeBtnActions() {
        if ($this->settings["actionbtn"]) {
            if ($this->settings["viewbtn"]) {
                $this->enqueueBtnActions("view", "javascript:;", "default", "<i class=\"fa fa-info-circle\"></i>", "", array("title" => $this->langData["view"]), "btn-info");
            }
            if ($this->settings["editbtn"]) {
                $this->enqueueBtnActions("edit", "javascript:;", "default", "<i class=\"fa fa-pencil-square-o\"></i>", "", array("title" => $this->langData["edit"]),"btn-warning");
            }
            if ($this->settings["inlineEditbtn"]) {
                $this->enqueueBtnActions("inline_edit", "javascript:;", "default", "<i class=\"fa fa-pencil\"></i>", "", array("title" => $this->langData["inline_edit"]),"btn-warning");
            }
            if ($this->settings["delbtn"]) {
                $this->enqueueBtnActions("delete", "javascript:;", "default", "<i class=\"fa fa-times fa-fw\"></i>", "", array("title" => $this->langData["delete"]),"btn-danger");
            }
        }
    }
    ...
    private function reorderColumn($results) {
        if (isset($this->colOrder) && count($this->colOrder) > 0) {
            $data = array();
    ...
    private function getHTMLElement($fieldName, $htmlType, $attr = array(), $data = array(), $fieldData = array(), $fieldClass = array()) {
        ...
        switch (strtoupper($htmlType)) {
            ...
            case "FILE":
                return $this->getInputField($fieldName, $attr, $data, "file", $fieldClass);
            case "FILE_NEW":
                return $this->getFileUploadField($fieldName, $attr, $data, "file", $fieldClass);
            case "FILE_MULTI":
                $fieldName = $fieldName . "[]";
                $attr = array_merge($attr, array(
                    "multiple" => "multiple"
    ...
    private function addWhereCondition(PDOModel $pdoModelObj, $data = array()) {
        ...
        if (isset($data["filter_data"]) && count($data["filter_data"])) {
            ...
            $this->filterData = $data;
        } else if (isset($this->filterData["filter_data"]) && count($this->filterData["filter_data"])) {
        ...
        if (isset($this->settings["resetSearch"]) && $this->settings["resetSearch"] === true && empty($this->search["search_text"]))
            $this->search = array();
        if (isset($this->settings["resetSearch"]) && $this->settings["resetSearch"] === true && count($this->filterData) === 0)
    ...
    public function getFileUploadField($fieldName, $attr, $data, $fieldData, $fieldClass = array()) {
        ...
        $fileUpload = $inputControl . $fileAddBtn . $fileRemoveBtn . $fileControlDiv;
        return $fileUpload;
    }
